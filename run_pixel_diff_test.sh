...
BUILD_DIR="${ROOT_DIR}/motion_pixel_diff/build"
RUN_DIR="${ROOT_DIR}/.run_pixel_diff"
...
if [[ ! -f "${VIDEO_PATH}" ]]; then
  ...
  echo "Uso: bash run_pixel_diff_test.sh [caminho-do-video.mp4]" >&2
  exit 1
fi
...
"${BUILD_DIR}/motion_pixel_diff" "${RTSP_URL}"
